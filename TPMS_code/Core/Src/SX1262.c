...
void SX1262CalculatePacketLoss(Radio* radio)
{
	...
	radio->per.rx_counter = (radio->rx_msg[0] << 24) | (radio->rx_msg[1] << 16) |
			                (radio->rx_msg[2] << 8)  |  radio->rx_msg[3];
...
void SX1262CreateTeleTxMsg(Radio* radio, uint8_t id)
{
	...
	radio->tx_msg[0] = radio->per.tx_counter >> 24;
	radio->tx_msg[1] = radio->per.tx_counter >> 16;
	radio->tx_msg[2] = radio->per.tx_counter >> 8;
	radio->tx_msg[3] = radio->per.tx_counter;

	// NOT IMPLEMENTED YET
	for (uint8_t i = 4; i < radio->packetParams.Params.Gfsk.PayloadLength - 2; ++i)
	{
		radio->tx_msg[i] = id;
	}
}
